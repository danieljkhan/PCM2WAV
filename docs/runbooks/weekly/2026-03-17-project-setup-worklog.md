...
### `c6f2aae` — [fix] 출력 디렉터리 미존재 시 wave.open 실패 버그 수정

- **변경 요약**:
  - `pcm2wav/converter.py` — `convert_pcm_to_wav()`에서 `wave.open()` 호출 전 `wav_path.parent.mkdir(parents=True, exist_ok=True)` 추가. 출력 디렉터리 미존재 시 `[Errno 2] No such file or directory` 방지.
  - `tests/test_converter.py` — 2개 테스트 추가:
    - `test_convert_creates_output_directory` — 단일 레벨 디렉터리 자동 생성 검증
    - `test_convert_creates_nested_output_directory` — 다중 레벨 디렉터리 자동 생성 검증
- **검증 결과**:
  - 3라운드 검증 체인: 전 라운드 PASS (BLOCK 0, WARN 2 — non-blocking)
  - QC: ruff check ✓, ruff format ✓, mypy strict ✓, pytest 89 passed / 1 skipped
  - Polish loop: 2 cycles (20 iterations) 수정 0건으로 종료 조건 충족
...
- [x] 버그 수정: 출력 디렉터리 미존재 시 변환 실패 → `mkdir` 자동 생성
...
- 출력 디렉터리 버그 수정 + 검증 파이프라인 통과: 2026-03-17 21:50 KST
